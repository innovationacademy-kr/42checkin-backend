...
const login = async (user: User): Promise<string> => {
	try {
		...
		return await authService.generateToken(existingUser ? existingUser : user);
	} catch (e) {
 ...
 * 유저 및 카드 체크인 처리
 */
const checkIn = async (id: number, cardId: string) => {
	try {
 ...
 * 강제 체크아웃
 */
const forceCheckOut = async (adminId: number, userId: string) => {
	try {
