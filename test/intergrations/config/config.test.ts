...
describe('config api test', async () => {
	...
	describe((`설정 테이블의 값을 조회`), () => {
		it('현재 환경의 이름값으로 값을 조회합니다.', async () => {
			...
			const res = await request(app).get(`/config`).query(query).set('Cookie', [sessionCookie]);
	...
	describe((`설정 테이블의 값을 수정`), () => {
		it('특정 날짜를 선택하여 최대입장 인원수를 수정하고 다시 되돌립니다.', async () => {
            const YYYYMMDD = getTimeFormat(new Date(), 'YYYY-MM-DD');
            const query = { date: YYYYMMDD };
            const { body: { gaepo: oldGaepoCnt } } = await request(app).get(`/config`).query(query).set('Cookie', [sessionCookie]);
            const maxCnt = 200;
            const body = {
                env: {
                    gaepo: maxCnt,
			...
			const res = await request(app).put(`/config`).send(body).set('Cookie', [sessionCookie]);
			...
			await request(app).put(`/config`).send(body).set('Cookie', [sessionCookie]);
		});

		it('특정 날짜를 선택하여 입장가능 시간을 수정하고 다시 되돌립니다.', async () => {
            const YYYYMMDD = getTimeFormat(new Date(), 'YYYY-MM-DD');
            const query = { date: YYYYMMDD };
            const { body: { begin_at: oldBegin_at } } = await request(app).get(`/config`).query(query).set('Cookie', [sessionCookie]);
            const date = new Date();
            const body = {
                env: {
                    begin_at: date,
